...
function render(){if(!$('chordLibraryGrid'))return;renderSelectors();const positions=allPositions(),notes=noteNames();$('libraryTitle').textContent=chordName();$('libraryNotes').textContent=`Notas: ${notes.map(formatNote).join(' · ')}`;$('chordLibraryGrid').innerHTML=positions.map((p,i)=>`<article class="library-card ${i===0?'featured':''}"><span class="library-rank">${i===0?'Más conocida':i===1?'Muy usada':`Posición ${i+1}`}</span><div class="library-card-head"><div><h3>${p.name}</h3><p>${p.description}</p></div><button class="library-card-play" data-library-play="${i}" type="button" aria-label="Escuchar ${p.name}"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7Z"/></svg></button></div>${diagram(p)}<div class="library-card-foot"><span>${p.base===0?'Abierto':`Desde traste ${p.base}`}</span></div></article>`).join('');$('chordLibraryGrid').querySelectorAll('[data-library-play]').forEach(btn=>btn.onclick=()=>play(positions[Number(btn.dataset.libraryPlay)]));$('playLibraryChord').onclick=()=>play(positions[0]);}
